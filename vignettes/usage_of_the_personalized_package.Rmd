...
\begin{eqnarray}
\Delta_{kl}(x) \equiv 
\{ E(Y | T = k, { X} = { x}) - E(Y | T = l, \bfX = { x}) \} \label{definition of Delta_kl}
\end{eqnarray}
If $\Delta_{kl}(x) > 0$, then treatment $k$ is preferable to treatment $l$ for a patient with $\bfX = \bfx$. For each patient, evaluation of all pairwise comparisons of the $\Delta_{kl}(x)$ indicates which treatment leads to the largest expected outcome. 
...
L_W(f_1, \dots, f_{K}) = \frac{1}{n}\sum_{i = 1}^n\frac{\boldsymbol M(Y_i,  \sum_{k = 1}^{K}I(T_{i} = k)\times f_k(x_i) ) }{ { Pr(T = T_i | \bfX = \bfx_i)} },
\end{equation}
